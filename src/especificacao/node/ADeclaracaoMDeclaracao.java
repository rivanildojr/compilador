/* This file was generated by SableCC (http://www.sablecc.org/). */

package especificacao.node;

import java.util.*;
import especificacao.analysis.*;

@SuppressWarnings("nls")
public final class ADeclaracaoMDeclaracao extends PDeclaracao
{
    private final LinkedList<PVar> _var_ = new LinkedList<PVar>();

    public ADeclaracaoMDeclaracao()
    {
        // Constructor
    }

    public ADeclaracaoMDeclaracao(
        @SuppressWarnings("hiding") List<?> _var_)
    {
        // Constructor
        setVar(_var_);

    }

    @Override
    public Object clone()
    {
        return new ADeclaracaoMDeclaracao(
            cloneList(this._var_));
    }

    @Override
    public void apply(Switch sw)
    {
        ((Analysis) sw).caseADeclaracaoMDeclaracao(this);
    }

    public LinkedList<PVar> getVar()
    {
        return this._var_;
    }

    public void setVar(List<?> list)
    {
        for(PVar e : this._var_)
        {
            e.parent(null);
        }
        this._var_.clear();

        for(Object obj_e : list)
        {
            PVar e = (PVar) obj_e;
            if(e.parent() != null)
            {
                e.parent().removeChild(e);
            }

            e.parent(this);
            this._var_.add(e);
        }
    }

    @Override
    public String toString()
    {
        return ""
            + toString(this._var_);
    }

    @Override
    void removeChild(@SuppressWarnings("unused") Node child)
    {
        // Remove child
        if(this._var_.remove(child))
        {
            return;
        }

        throw new RuntimeException("Not a child.");
    }

    @Override
    void replaceChild(@SuppressWarnings("unused") Node oldChild, @SuppressWarnings("unused") Node newChild)
    {
        // Replace child
        for(ListIterator<PVar> i = this._var_.listIterator(); i.hasNext();)
        {
            if(i.next() == oldChild)
            {
                if(newChild != null)
                {
                    i.set((PVar) newChild);
                    newChild.parent(this);
                    oldChild.parent(null);
                    return;
                }

                i.remove();
                oldChild.parent(null);
                return;
            }
        }

        throw new RuntimeException("Not a child.");
    }
}
